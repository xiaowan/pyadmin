...
class ToolService(BaseService):
    ...
    def __init__(self):
        ...
        super(ToolService, self).__init__()
    ...
    def get_schema(self, dbname='', table='', superclass='', prefix="yes"):
        ...
        data.append("\n")
        data.append("\n")
        data.append("class {name}DO({superclass}):".format(name=self.toolDAO.get_schema_name(table, prefix), superclass=superclass))
        ...
        if schema:
            for x in schema:
                ...
                if x['default_value']:
                    condition.append("default={value}".format(value=x["default_value"]))

                fields.append({
                    "key": x["column_name"],
                    "val": "    {column_name} = Column({condition})".format(column_name=x["column_name"], condition=", ".join(condition))
                })
